...
t3t4t5_heatmap <- data.frame(aggregate(t3t4t5_freq, by= list(unique.values = t3t4t5_freq$tactics), FUN= length))
t3t4t5_heatmap <- t3t4t5_heatmap[which(t3t4t5_heatmap$Freq >= 50), 1]
t3t4t5_heatmap_name <- as.data.frame(droplevels(t3t4t5_heatmap))
colnames(t3t4t5_heatmap_name)[1] <- "tactics"

t3t4t5_freq$iyear <- as.numeric(as.character(t3t4t5_freq$iyear))

t3t4t5_freq$tactics <- as.character(t3t4t5_freq$tactics)

y <- 1
for(i in 1:nrow(t3t4t5_heatmap_name)) {
  df <- t3t4t5_freq[which(t3t4t5_freq$tactics %in% t3t4t5_heatmap_name$tactics[i]), ]
...
t3t4t5_list <- lapply(ls(pattern="tactics_hm1"), function(x) get(x))
...
create_heatmap <- function(list_element, df) {
  ...
  heatmap <- ggplot(data = list_element, mapping = aes(x = iyear, y = region_txt, mean)) +
  ...
  figure <- ggdraw() +
    ...
    draw_plot_label(label = c("Number of overall attacks in 1998-2017", "Number of attacks specified in the title", "Only frequencies >0 are considered for the analysis"), size = 10,
...
t <- 311
for (i in t3t4t5_list) {
  title <- i[[2]][[1]]
  create_heatmap(i, t3t4t5_freq)
  t <- t+1
}
