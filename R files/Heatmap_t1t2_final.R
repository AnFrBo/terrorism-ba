...
t1t2_heatmap <- data.frame(aggregate(t1t2_freq, by= list(unique.values = t1t2_freq$tactics), FUN= length))
t1t2_heatmap <- t1t2_heatmap[which(t1t2_heatmap$Freq >= 50), 1]
t1t2_heatmap_name <- as.data.frame(droplevels(t1t2_heatmap))
colnames(t1t2_heatmap_name)[1] <- "tactics"

t1t2_freq$iyear <- as.numeric(as.character(t1t2_freq$iyear))

t1t2_freq$tactics <- as.character(t1t2_freq$tactics)

y <- 1
for(i in 1:nrow(t1t2_heatmap_name)) {
  df <- t1t2_freq[which(t1t2_freq$tactics %in% t1t2_heatmap_name$tactics[i]), ]
...
t1t2_list <- lapply(ls(pattern="tactics_hm1"), function(x) get(x))
...
create_heatmap <- function(list_element, df) {
  ...
  figure <- ggdraw() +
    ...
    draw_plot_label(label = c("Number of overall attacks in 1977-1997", "Number of attacks specified in the title", "Only frequencies >0 are considered for the analysis"), size = 10,
